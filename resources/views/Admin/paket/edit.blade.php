...
@section('main')
    <div class="mx-4">
        <h2 class="ms-4 mb-3  font-bold tracking-tight text-gray-500 ">Paket / Tambah </h2>
        <p class="ms-4 text-3xl font-semibold tracking-tight text-gray-900 dark:text-white"> Tambah Paket Wisata </p>
        <form class="mx-auto" action="/admin/paket/{{ $paket['id'] }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <div
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
                    <input type="hidden" name="gambarOld" value="{{ $paket['gambar'] }}">
                    <div class="mb-5">
                        <label for="nama"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama</label>
                        <input type="text" value="{{ $paket['nama'], old('nama') }}" name="nama" id="nama"
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" value="{{ $paket['harga'], old('harga') }}" name="harga" id="harga"
                            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                            required />
                    </div>
                </div>
                <div class="mb-5">
                    ...
                    <textarea id="text" name="deskripsi" rows="4"
                        ...
                        placeholder="Write your thoughts here...">{{ $paket['deskripsi'], old('deskripsi') }}</textarea>
                </div>
            </div>
